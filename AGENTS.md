...
- **Standard ISOBMFF box content parsing implementation**: Added comprehensive content parsing for standard ISO Base Media File Format boxes beyond iTunes metadata
- **Reasoning**: Created new `src/isobmff_content.rs` module (760+ lines) providing detailed parsing and display for 15+ standard ISOBMFF box types. Implemented parsing for ftyp (File Type with brand info), mvhd (Movie Header with timescale, duration, rate, volume), tkhd (Track Header with dimensions, volume, flags), mdhd (Media Header with timescale, duration, language), hdlr (Handler Reference with track type identification), vmhd/smhd (Video/Sound Media Headers), dref (Data Reference with entry count), stsd (Sample Description with codec entries), stts/stsc/stsz/stco/co64 (sample table boxes with entry counts), elst (Edit List), and url /urn  (data entry boxes). Enhanced `IsobmffBox` struct to support both iTunes metadata (`itunes_content`) and standard box content (`content`) fields. Added special handling for FullBox containers (meta, dref) which have version/flags headers before their children, with dref requiring 8-byte skip (version/flags + entry_count) before box children. Updated container detection to include dref, sinf, rinf, trgr, grpl, and schi. All parsed content displays with consistent formatting matching ID3v2 frame display style, showing version numbers, flags with bit interpretation, timing information with calculated durations in seconds, human-readable handler types (vide=Video Track, soun=Audio Track, text=Text/Subtitle Track), and codec format lists. Tested with M4A podcast file showing complete parsing of all major box types with detailed diagnostic information. This brings ISOBMFF support to feature parity with ID3v2, providing comprehensive media file analysis across both format types.
- **Handler Reference manufacturer field parsing**: Enhanced hdlr box parsing to extract and display manufacturer code from reserved field
